/*
 * floatPoint_preEchoTest.c
 *
 *  Created on: Oct 26, 2012
 *      Author: BLEE
 *
 *  Description: This is the test program for floating-point MDCT/IMDCT/pre-echo experiment
 *
...
void main(void)
{
    Int16  j,init,M,c;
    Uint32 cnt;
    FILE   *f_org;               // File of speech data
    FILE   *f_prc;               // File of serial bits for transmission
    ...
    if (c == 2)
    {
    	f_org = fopen("..\\data\\dtmf_digit2.wav", "rb");
    	f_prc = fopen("..\\data\\preEchoProc.wav", "wb");
    }
    else
    {
    	f_org = fopen("..\\data\\dtmf_digit2.pcm", "rb");
    	f_prc = fopen("..\\data\\preEchoProc.pcm", "wb");
    }
    ...
    if (FRAME == 512)
    {
    	printf("Note: if FRAME = 512, it takes more time to process the samples\n");
    }
    cnt = 0;
    while (fread(temp, sizeof(Int8), 2*M, f_org) == (2*M))
    {
		...
		mdct_qnt( mdct_enc16, M, QuantStep);     // Simulate Log Quantization
