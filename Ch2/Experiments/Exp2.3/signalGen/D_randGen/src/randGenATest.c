...
void main(void)
{
    ...
    printf("Exp --- D: random number generation experiment\n");

	printf("Play random noise using assembly randNumber()\n");
	playAudio();
...
void leftChannel_process(Int16 *output, Int16 size)
{
	...
	for(i=0; i<size; i++)
	{
		*(output + i) = randNumber();	// Send random noise through left channel
	}
}
